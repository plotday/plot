 ...
 * GitHub Issues tool
 *
 * Implements the ProjectTool interface for syncing GitHub Issues
 * with Plot activities. Explicitly filters out pull requests.
 */
export class GitHubIssues extends Tool<GitHubIssues> implements ProjectTool {
  ...
  /**
   * Create GitHub API client using syncable-based auth
   */
  private async getClient(syncableId: string): Promise<Octokit> {
  ...
  /**
   * Returns available GitHub repos as syncable resources.
   */
  async getSyncables(
    _auth: Authorization,
    token: AuthToken
  ...
  async onSyncEnabled(syncable: Syncable): Promise<void> {
    ...
    // syncable.title is "owner/repo" (full_name)
    const [owner, repo] = (syncable.title ?? "").split("/");
    if (owner && repo) {
      await this.set<RepoInfo>(`repo_info_${syncable.id}`, {
        owner,
        repo,
        fullName: syncable.title ?? "",
      });
    }
  ...
  async onSyncDisabled(syncable: Syncable): Promise<void> {
    ...
    await this.clear(`repo_info_${syncable.id}`);
  }
  ...
  private async syncBatch(
    ...
    const callbackToken = await this.get<Callback>(
      `item_callback_${repoId}`
    );
    if (!callbackToken) {
      throw new Error(`Callback token not found for repo ${repoId}`);
    }
  ...
  async updateIssue(
    ...
    if (thread.assignee) {
      ...
      if (actor?.name) {
        ...
        updateFields.assignees = [actor.name];
      }
    } else {
  ...
  private async onWebhook(
    ...
    if (!callbackToken) {
      console.warn("No callback token found for repo:", repoId);
      return;
    }
    ...
    if (event === "issues") {
      await this.handleIssueWebhook(payload, repoId, callbackToken);
    } else if (event === "issue_comment") {
      await this.handleCommentWebhook(payload, repoId, callbackToken);
    }
  }
