...
export type NewSchedule = {
  ...
  | {
      /**
       * Unique identifier for the schedule, generated by Uuid.Generate().
       * Specifying an ID allows tools to track and upsert schedules.
       */
      id: Uuid;
    }
  | {
      /* No id required. An id will be generated and returned. */
    }
);

/**
 * Type for updating existing schedules.
 *
 * Must provide `id` to identify the schedule. All other fields are optional
 * and only provided fields will be updated.
 *
 * @example
 * ```typescript
 * // Reschedule
 * await plot.updateSchedule({
 *   id: scheduleId,
...
export type ScheduleUpdate = {
  id: Uuid;
} & Partial<Omit<NewSchedule, "threadId" | "id">>;
